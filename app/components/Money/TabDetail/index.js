...
class TabDetail extends React.Component { 
  ...
  convertError(r){
    if(r.description.indexOf("listCards")>=0){
      if(r.error===0){
  ...
  render() {
            ...
            if(index==0){
              if(row.length >0){
                ...
                ncc = row.map((r,i)=>{
                 ...
                 total_content =  (<i><h3 style={{marginLeft:'5%',marginTop:'2%'}}>Tổng tiền theo nhà mạng : {this.formatCurency(t_total)}</h3></i>);
              }else{
                header_ncc=(
                  <tr style={{background:'rgb(236, 246, 253)',fontStyle:'italic'}}>
                    <th style={{height:50,border:'1px solid #e2e2e2',fontSize:13,textAlign:'center'}}>Nhà mạng</th>
                ...
                if(index >= (this.state.activePage-1) *itemsPerPage && index < (this.state.activePage) *itemsPerPage ){
                  ...
                  item = row.array.map((r,i)=>{  
                    ...
                    return(
                      <tr key={i}>
                        ...
                        <td style={{height:40,border:'1px solid #e2e2e2',fontSize:13,textAlign:'center'}}>{this.formatCurency(r.amount)}</td>
                        <td style={{height:40,border:'1px solid #e2e2e2',fontSize:13,textAlign:'center'}}>{r.providerCard?(r.providerCard=="epay"?"epay":"ngân lượng"):"Chưa cập nhật"}</td>              
                  ...
                  var temp=(
                    <div key={index} style={{marginTop:'2%'}}>
                      ...
                      <h3 style={{marginLeft:'5%'}}>Ngày : {date}</h3>
                      <i><h4 style={{marginLeft:'5%'}}>Tổng tiền trong ngày : {total}</h4></i>
                      <div style={{width:'100%',overflow:'auto',height:item_tt>7?'400px':'auto'}}>
                        <table style={{marginLeft:'5%',height:'100%',width:'95%'}}>
                        <tbody>
                          <tr style={{background:'rgb(236, 246, 253)',fontStyle:'italic'}}>
                            <th style={{height:50,border:'1px solid #e2e2e2',fontSize:14,textAlign:'center'}}>STT</th>                            
                            <th style={{height:50,border:'1px solid #e2e2e2',fontSize:14,textAlign:'center'}}>Nickname</th>
                            <th style={{height:50,border:'1px solid #e2e2e2',fontSize:14,textAlign:'center'}}>Số tiền</th>
                            <th style={{height:50,border:'1px solid #e2e2e2',fontSize:14,textAlign:'center'}}>NCC thẻ</th>
                            <th style={{height:50,border:'1px solid #e2e2e2',fontSize:14,textAlign:'center'}}>Mã lỗi</th>
                            <th style={{height:50,border:'1px solid #e2e2e2',fontSize:14,textAlign:'center'}}>Thời gian</th>
                            <th style={{height:50,border:'1px solid #e2e2e2',fontSize:14,textAlign:'center'}}>Chi tiết</th>
                          </tr>
                          {item}
                        </tbody> 
                        </table>
    ...
    return (
      <div>
        {total_content}
        <table style={{width:'95%',marginLeft:'5%',marginTop:'1%'}}>

...
class Dauhistory extends React.Component { 
  constructor(props) {
    super(props);
    this.state = {     
      activePage :1, 
  ...
  onChange = (page) => {
    ...
    if(_date &&_todate){
      ...
      this.props.count_dau(someDate,toDate,page);
      this.props.is_load_dau(false);
  ...
  onClick=()=>{
    ...
    if(_date &&_todate){
      var someDate = new Date(_date).getTime();
      var toDate = new Date(_todate).getTime();
      this.props.count_dau(someDate,toDate,1);
      this.props.is_load_dau(true);
      this.setState({activePage:1});
      this.setState({activePageDAU:1});
  ...
  render() {
    ...
    var search = (
      <table style={{marginLeft:'0%',marginTop:'1%'}}>
        <tbody>
          <tr>
            <td>Ngày bắt đầu
            </td>
        ...
        if(this.props.isLoadDAU){
          ...
          if(this.props.data && this.props.data.length >0){
            ...
            if(this.state.chartDAU ){
              contentChart=(
                <div style={{width:'90%',border:'1px solid #e2e2e2',padding:'1%',marginLeft:'0%',marginBottom:'2%'}}>
                   <Bar
...
Dauhistory.propTypes = {

};

export default Dauhistory;

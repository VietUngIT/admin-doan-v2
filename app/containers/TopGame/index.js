...
export class TopGame extends React.Component { 
  constructor(props) {
    super(props);
    this.state = {
      win_fromdate : moment(),
    ...
    this.onChangeWinFromDate = this.onChangeWinFromDate.bind(this);
    this.onChangeWinToDate = this.onChangeWinToDate.bind(this);
  ...
  componentWillReceiveProps=(nextProps)=>{
    ...
    if(this.props.data_win_user != nextProps.data_win_user){
      nextProps.data_win_user.map((row,index)=>{
        if((gn_==1 )){
          if(row.bank){
            data_print2.push(
              {
        ...
        if((gn_==13 || gn_==14 )){
          if(row.caothap!=="0/0/0/0/0"){
            // console.log(row.caothap)
            data_print2.push(
              {
                "r1":row.nickName,  
                "r6":"CaoThap",  
                "r2":(Math.abs(row.caothap.split("/")[4])),  
                "r3":(row.caothap.split("/")[1]),                   
            ...
            data_print2.push(
              {
                "r1":row.nickName,  
                "r6":"Chuyển khoản",  
                "r2":(Math.abs(row.chuyenkhoan.split("/")[0])),  
    ...
    if(this.props.data_win_bot != nextProps.data_win_bot){
      nextProps.data_win_bot.map((row,index)=>{
        if((gn_==1 )){
          if(row.bank){
            data_print3.push(
              {
  ...
  sortTotal=(data_print2)=>{
    ...
    this.handleSortTotal(data_print2);
  }
  handleSortTotal=(data_print2)=>{
    if(this.state.sort_total=="asc" || this.state.sort_total==""){
     
      data_print2 =(data_print2.sort(function (a, b) {
        if (parseInt(a.r3) < parseInt(b.r3)) return -1;
        else if (parseInt(a.r3) > parseInt(b.r3)) return 1;
        return 0;
      }));
  ...
  sortLoiNhuan=(data_print2)=>{
    ...
    this.handleSortLoiNhuan(data_print2);
  }
  handleSortLoiNhuan=(data_print2)=>{
    if(this.state.sort_loinhuan=="asc" || this.state.sort_loinhuan==""){
     
      data_print2 =(data_print2.sort(function (a, b) {
        if (parseInt(a.r4) < parseInt(b.r4)) return -1;
        else if (parseInt(a.r4) > parseInt(b.r4)) return 1;
        return 0;
      }));
  ...
  sortLuongTien=(data_print2)=>{
    ...
    this.handleSortLuongTien(data_print2);
  }
  handleSortLuongTien=(data_print2)=>{
    
    if(this.state.sort_luongtien=="asc" || this.state.sort_luongtien==""){
     
      data_print2 =(data_print2.sort(function (a, b) {
        if (parseInt(a.r2) < parseInt(b.r2)) return -1;
        else if (parseInt(a.r2) > parseInt(b.r2)) return 1;
        return 0;
      }));
  ...
  onChangeWinFromDate=(date)=>{
    this.setState({
      win_fromdate: date,
    });
     
  }
  onChangeWinToDate=(date)=>{
    this.setState({
      win_todate: date,
    });
  ...
  top_win=()=>{
    ...
    if(this.refs.w_fromDate.input.props.value.toString().trim() && this.refs.w_fromDate.input.props.value.toString().trim()!==""
    && this.refs.w_toDate.input.props.value.toString().trim() && this.refs.w_toDate.input.props.value.toString().trim()!=="")
    {
      if(this.convertDate(new Date().getTime()) ===this.refs.w_toDate.input.props.value.toString().trim()){
        toDate = new Date().getTime();
      }else{
        _todate = this.refs.w_toDate.input.props.value.toString().trim().split('/').reverse().join('/')+ " 23:59:59";
        toDate = new Date(_todate).getTime();
      }
     _date = this.refs.w_fromDate.input.props.value.toString().trim().split('/').reverse().join('/')+ " 00:00:00";
     someDate = new Date(_date).getTime();
     st_win = this.refs.w_fromDate.input.props.value.toString().trim();
     et_win = this.refs.w_toDate.input.props.value.toString().trim() ;
  
      this.props.top_win(this.state.un,g_name,someDate,toDate);
  ...
  fileUser=()=>{
    message.warning("Chờ trong giây lát !")
  }
  changeTab=()=>{
    this.setState({sort_loinhuan:"asc"})
    this.setState({isSortLoiNhuan:false})
  ...
  render() {
    ...
    if(this.props.loadWin){
      ...
      if(this.props.data_win_user || this.props.data_win_user){
        if(this.props.data_win_user && this.props.data_win_user.length > 0){
                  ...
                  if(gn_>=7 && gn_<=48 ){
                    return(
                      <tr key={indexs}>
                        ...
                        <td style={{height:40,border:'1px solid #e2e2e2',fontSize:13,textAlign:'left',padding:5,display:(gn_!=7&&gn_!=8)?"":"none"}}>{rows.r5}</td>
                      </tr>
          ...
          if((gn_==1 ||gn_==2 ||gn_==3||gn_==4||gn_==5||gn_==6||gn_==50)){
                      ...
                      <table style={{width:'95%',marginBottom:30}} >
                        <tbody>
                          <tr style={{background:'#ecf6fd',fontStyle:'italic'}}>
                            <th style={{height:40,border:'1px solid #e2e2e2',fontSize:13,textAlign:'left',padding:5}} ><Link >STT </Link> </th>
                            <th style={{height:40,border:'1px solid #e2e2e2',fontSize:13,textAlign:'left',padding:5}} ><Link >Nickname</Link> </th>
                            <th style={{height:40,border:'1px solid #e2e2e2',fontSize:13,textAlign:'left',padding:5}} >
                            <Link onClick={()=>this.sortLuongTien(data_print2)}>Số tiền
                          ...
                          <th style={{height:40,border:'1px solid #e2e2e2',fontSize:13,textAlign:'left',padding:5}} ><Link >Số giao dịch</Link> </th>
                          </tr> 
                          {body_table}
                        </tbody>
                      </table>
              ...
              print1 = (
                <div style={{marginBottom:10}}>
                  <Workbook filename={"Top_Win_User_"+st_win+"_"+et_win+".xlsx"} element={<Button type="primary">Xuất file</Button>}>
                  ...
                  <table style={{width:'95%',marginBottom:30}} >
                    <tbody>
                      <tr style={{background:'#ecf6fd',fontStyle:'italic'}}>
                        ...
                        <th style={{height:40,border:'1px solid #e2e2e2',fontSize:13,textAlign:'left',padding:5}}>
                          <Link onClick={()=>this.sortLuongTien(data_print2)}>Luồng tiền
                        ...
                        <th style={{height:40,border:'1px solid #e2e2e2',fontSize:13,textAlign:'left',padding:5}}>
                          <Link onClick={()=>this.sortTotal(data_print2)}>Tiền user nhận
                        ...
                        <th style={{height:40,border:'1px solid #e2e2e2',fontSize:13,textAlign:'left',padding:5}}>
                          <Link onClick={()=>this.sortLoiNhuan(data_print2)}>Lợi nhuận
                        ...
                        <th style={{height:40,border:'1px solid #e2e2e2',fontSize:13,textAlign:'left',padding:5,fontStyle:'italic',display:(gn_!=7&&gn_!=8)?"":"none"}} >
                          <Link >Số giao dịch</Link>
                        </th>
                        ...
                        <th style={{height:40,border:'1px solid #e2e2e2',fontSize:13,textAlign:'left',padding:5,fontStyle:'italic'}} >Luồng tiền</th>
                        <th style={{height:40,border:'1px solid #e2e2e2',fontSize:13,textAlign:'left',padding:5,fontStyle:'italic'}} >Tiền user nhận</th>
                        <th style={{height:40,border:'1px solid #e2e2e2',fontSize:13,textAlign:'left',padding:5,fontStyle:'italic'}} >Lợi nhuận</th> */}
                      </tr>  
                      {body_table}
                    </tbody>
                  </table>
                ...
                <div style={{marginBottom:10}}>
                  <Workbook filename={"Top_Win_User_"+st_win+"_"+et_win+".xlsx"} element={<Button type="primary">Xuất file</Button>}>
                    <Workbook.Sheet data={data_print2} name="Top game">
                      ...
                      <Workbook.Column label="Luồng tiền" value={r=>this.formatCurency(r.r2)}/>             
                      <Workbook.Column label="Tiền user nhận" value={r=>this.formatCurency(r.r3)}/>             
                      <Workbook.Column label="Lợi nhuận" value={r=>this.formatCurency(r.r4)}/>             
        ...
        if(this.props.data_win_bot && this.props.data_win_bot.length > 0){
        ...
        if((gn_==1 ||gn_==2 ||gn_==3||gn_==4||gn_==5||gn_==6||gn_==50)){
                    ...
                    <table style={{width:'95%',marginBottom:30}} >
                      <tbody>
                        <tr style={{background:'#ecf6fd',fontStyle:'italic'}}>
                          ...
                          <th style={{height:40,border:'1px solid #e2e2e2',fontSize:13,textAlign:'left',padding:5}} ><Link >Nickname</Link> </th>
                          <th style={{height:40,border:'1px solid #e2e2e2',fontSize:13,textAlign:'left',padding:5}} >
                            <Link onClick={()=>this.sortLuongTien(data_print3)}>Số tiền
            ...
            print2 = (
              <div style={{marginBottom:10}}>
                <Workbook filename={"Top_Win_Bot_"+st_win+"_"+et_win+".xlsx"} element={<Button type="primary">Xuất file</Button>}>
                ...
                <table style={{width:'95%',marginBottom:30}} >
                  <tbody>
                    ...
                    <th style={{height:40,border:'1px solid #e2e2e2',fontSize:13,textAlign:'center'}}>
                        ...
                        <th style={{height:40,border:'1px solid #e2e2e2',fontSize:13,textAlign:'left',padding:5}}>
                          <Link onClick={()=>this.sortLuongTien(data_print3)}>Luồng tiền
                        ...
                        <th style={{height:40,border:'1px solid #e2e2e2',fontSize:13,textAlign:'left',padding:5}}>
                          <Link onClick={()=>this.sortTotal(data_print3)}>Tiền user nhận
                              {this.state.isSortTotal?
                              (<Icon style={{marginLeft: '5px'}} type={this.state.isSortTotal? (this.state.sort_total=="desc"?"up-square-o":"down-square-o"):""}></Icon>):
                              <img src={require('images/sort.png')} style={{width:13,height:13,marginLeft:5}}/>}                   
                          </Link>
                        ...
                        <th style={{height:40,border:'1px solid #e2e2e2',fontSize:13,textAlign:'left',padding:5}}>
                          <Link onClick={()=>this.sortLoiNhuan(data_print3)}>Lợi nhuận
    ...
    return (
      <div>
            <h3 style={{marginLeft:'5%',marginTop:'2%',marginBottom:'2%'}}>Top game</h3>
            <div style={{overflow:'auto'}}>
              <table style={{marginLeft:'5%',marginTop:10,marginBottom:10}}>  
              <tbody>
                <tr>
                  <td>Ngày bắt đầu
                  </td>
                ...
                <tr>
                  <td>
                    <DatePicker
                      style={{color:'red'}}
                      ref="w_fromDate"
                      selected={this.state.win_fromdate}
                      onChange={this.onChangeWinFromDate}
                  ...
                  <td>
                    ...
                    style={{color:'red'}}
                    ref="w_toDate"
                    selected={this.state.win_todate}
                    onChange={this.onChangeWinToDate}
                    customInput={<Input  style={{width:'80%',height:'35px',border:'1px solid #e2e2e2',borderRadius:'5px',background:'white'}}/>}
                    dateFormat="DD/MM/YYYY"
                    showYearDropdown
                    scrollableYearDropdown
                ...
                <tr>
                  ...
                  <td>
                    <Button icon="search" style={{marginLeft:'0%',height:35,width:35}} onClick={this.top_win}/>
...
const mapStateToProps = createStructuredSelector({
  data_win_user : selectDataWinUser(),
  data_win_bot : selectDataWinBot(),
  loadWin : selectisLoadWin(),
});

function mapDispatchToProps(dispatch) {
  return {
    top_win:(un,gn,st,et)=> dispatch(top_win(un,gn,st,et)),
    dispatch,
  };
}

export default withRouter(connect(mapStateToProps, mapDispatchToProps)(TopGame));
